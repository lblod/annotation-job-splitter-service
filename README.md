...
The service will respond with a `200` if it could successfully parse the received delta message and create the necessary tasks based on its configuration. This does **not** mean that all task resources have been inserted into the triplestore. Inserting a large amount of task resources takes some time, we opted not to keep the connection open the entire time. The status of the input tasks will be updated to `success` once all new tasks have been inserted.
